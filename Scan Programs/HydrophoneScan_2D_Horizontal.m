...
raster.home = [27.45   30   11.76]; % home position [x,y,z] in mm     % CHECK
raster.size = [3 3 0]; % [X,Y] in mm                      % CHECK
raster.step = [1/4,1/4,1]*wavelength; % [dx,dy] mm - must be greater than zero          % CHECK
...
NPoints = length(raster.xs)*length(raster.ys);
